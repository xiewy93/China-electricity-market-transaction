...
class  set_action():
    ...
    def get_variables(self,variables_names):
    ...
    def pre_data(self):
        buy = self.init_participants(input_buy,False)
        sell = self.init_participants(input_sell,True)
        return buy,sell
    ...
    def merge_data(self,pre_data,left_on):
        # merge(self.init_participants(input_buy,False),'buy_price') or merge(self.init_participants(input_sell,True),'sell_price')
    ...
    def assign_sum_deal_quantity(self,merged_data):
        # assign_sum_deal_quantity(self.merge(self.input_buy,'buy_price')) or  assign_sum_deal_quantity(self.merge(self.input_sell,'sell_price'))      
        assign_quantity = []
    ...
    def mean_voidance(self,assigned_data):
        # mean_voidance(self.assign_sum_deal_quantity(self.merge(self.input_buy,'buy_price')))
        # or mean_voidance(self.assign_sum_deal_quantity(self.merge(self.input_sell,'sell_price')))
    ...
    def unified_voidance(self,mean_voidance_data):
        # unified_voidance(self.mean_voidance(self.assign_sum_deal_quantity(self.merge(self.input_buy,'buy_price'))))
        # or unified_voidance(self.mean_voidance(self.assign_sum_deal_quantity(self.merge(self.input_sell,'sell_price'))))
    ...
    def return_undeal( self,pre_data,assign_quantity_result):
        ...
        i=j=0
        for k in range(len(participants_quantity)):
            if sum(participants_quantity[i]) > sum(assign_quantity_result.assign_quantity[j]):
    ...
    def get_picture(self):
        x, buy_price = self.pre_data_for_picture(self.init_participants(input_buy,False)) 
        y, sell_price = self.pre_data_for_picture(self.init_participants(input_sell,True))
...
#        return buyer_mean_voidance_result , seller_mean_voidance_result
...
#        return buyer_unified_voidance_result , seller_unified_voidance_result        
...
#        return return_buyer_undeal , return_seller_undeal
        get_picture = self.get_picture()
#        return get_picture
        return buy,sell,compute_sum_deal_quantity,buy_deal_data, sell_deal_data,assign_buy_quantity_result , assign_sell_quantity_result,buyer_mean_voidance_result , seller_mean_voidance_result,buyer_unified_voidance_result , seller_unified_voidance_result,return_buyer_undeal , return_seller_undeal,get_picture
...
if __name__=='__main__':
    ...
    pre_action = set_action(input_sell,input_buy)
